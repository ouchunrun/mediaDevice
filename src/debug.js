...
module.exports = function setup(env) {
    ...
    // create dataBase
    createDebug.dataBaseListDB = new window.DBmanager('DatabaseLists', "keyvaluepairs", 1, ["dbName", "TS"]);
    createDebug.localLogsDB = new window.DBmanager(env.sessionStorage.dbName, "localLogs", 1, ["cseqNumber", "moduleName", "logLevel", "TS", "content"]);
   ...
   */
    ...
    function getLocalLogs(dbName, filter, callback) {
        ...
        if (dbName && dbName !== createDebug.sessionStorage.dbName) {
            db = new window.DBmanager(dbName, "localLogs", 1, ["cseqNumber", "moduleName", "logLevel", "TS", "content"]);
        } else {
        ...
        if (filter !== '*') {
            ...
            var request = window.indexedDB.open(db.dbName);
    ...
    function getLocalDBName(callback) {
        var localDBs = [];
        var request = window.indexedDB.open(createDebug.dataBaseListDB.dbName);
           ...
           function(logs) {
               if ( navigator.userAgent.match('(rv:11.0|Edge)') ) {
                   ...
                   if (window.navigator && window.navigator.msSaveBlob) {
                       window.navigator.msSaveBlob(log_file, fileName);
                   }
